...
router.post('/:gameId/queue', authMiddleware, (req: AuthedRequest, res: Response) => {
  const engine = getEngine(req.params.gameId as string);
  if (!engine) { res.status(404).json({ error: 'Game not found' }); return; }
  const result = joinQueue(req.params.gameId as string, req.agent!.id);
  res.json(result);
});

router.delete('/:gameId/queue', authMiddleware, (req: AuthedRequest, res: Response) => {
  leaveQueue(req.params.gameId as string, req.agent!.id);
  res.json({ ok: true });
});
...
router.post('/:gameId/challenge', authMiddleware, (req: AuthedRequest, res: Response) => {
  const engine = getEngine(req.params.gameId as string);
  if (!engine) { res.status(404).json({ error: 'Game not found' }); return; }
  const sessionId = createChallenge(req.params.gameId as string, req.agent!.id);
  res.json({ session_id: sessionId, game_id: req.params.gameId as string });
});

router.post('/:gameId/join/:sessionId', authMiddleware, (req: AuthedRequest, res: Response) => {
  const result = joinChallenge(req.params.sessionId as string, req.agent!.id);
